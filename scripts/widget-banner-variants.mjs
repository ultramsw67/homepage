// 블로그 사이드바 위젯 배너 CTA 문구 시안 생성 (widget-banner.mjs 와 같은 디자인, CTA·보조문구만 교체)
// 사용: node scripts/widget-banner-variants.mjs "C:/Users/ultramsw67/Desktop/블로그 위젯 시안"
...
const W = 510, H = 759;
const outDir = process.argv[2] || path.join(os.homedir(), 'Desktop', '블로그 위젯 시안');
fs.mkdirSync(outDir, { recursive: true });

// axes: 버튼 위 두 줄 / cta: 버튼 문구
const variants = [
  { name: '시안1 칼럼 아카이브', axes: '사업모델 · 지표 · 투자유치<br><b>매일 아침</b> 경영 칼럼 한 편', cta: '칼럼 200편 모아보기' },
  { name: '시안2 다음 한 수', axes: '사업모델 · 지표 · 투자유치<br>초기 창업자의 <b>다음 한 수</b>', cta: '다음 한 수 보러 가기' },
  { name: '시안3 자문 사례', axes: '사업모델 · 지표 · 투자유치<br><b>스무 팀</b>과 함께 푼 문제들', cta: '자문 사례 보러 가기' },
  { name: '시안4 둘러보기', axes: '사업모델 · 지표 · 투자유치<br><b>1인 기업</b> 첫 매출까지', cta: '수드 홈페이지 둘러보기' },
  { name: '시안5 질문형', axes: '사업모델 · 지표 · 투자유치<br><b>1인 기업</b> 첫 매출까지', cta: '지금 뭘 먼저 해야 할까?' },
];

const page = (v) => {
  // 버튼 글자 수에 따라 크기만 조금 줄인다 (가로 넘침 방지)
  const len = v.cta.length;
  const ctaSize = len <= 10 ? 31 : len <= 12 ? 28 : 26;
...
:root{--navy:#0b1f3a;--ivory:#f6f3ec;--brass:#b8924a;--brass-lit:#d3b071;--muted:rgba(246,243,236,.72)}
...
.cta{margin-top:auto;width:100%;height:92px;border-radius:4px;background:linear-gradient(180deg,#d3b071,#b8924a);color:#0b1f3a;
  font-size:${ctaSize}px;font-weight:700;letter-spacing:-.02em;display:flex;align-items:center;justify-content:center;gap:10px;
  white-space:nowrap;box-shadow:0 10px 30px -12px rgba(0,0,0,.6)}
.cta span{font-size:${ctaSize - 1}px}
...
<div class="card"><div class="frame"></div>
  ...
  <div class="axes">${v.axes}</div>
  <div class="cta">${v.cta} <span>→</span></div>
  <div class="url">soodcoach.com</div>
</div></body></html>`;
};

for (const v of variants) {
  const tmp = path.join(os.tmpdir(), `sood-widget-${Buffer.from(v.name).toString('hex').slice(0, 12)}.html`);
  fs.writeFileSync(tmp, page(v), 'utf8');
  ...
  execFileSync(CHROME, ['--headless=new', '--disable-gpu', '--hide-scrollbars', '--no-first-run', '--no-default-browser-check',
    `--window-size=${W},${H}`, '--force-device-scale-factor=1', '--virtual-time-budget=8000', `--screenshot=${out}`, 'file:///' + tmp.replace(/\\/g, '/')],
